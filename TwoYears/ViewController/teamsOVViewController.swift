...
//        orangeLabel.text = String(Int(orangeViewCount!)) + " Pt"
//        violetLabel.text = String(Int(violetViewCount!)) + " Pt"
...
//            self.underView.transform = CGAffineTransform(scaleX: 1, y: 0.1)
            self.underView.transform = CGAffineTransform(translationX: 0, y: moveheight * 0.05 * CGFloat(self.minusInt!))
            
        }) { bool in
            // ②アイコンを大きくする
            UIView.animate(withDuration: 0.2, delay: 0, animations: {
//                self.orangeLabel.transform = CGAffineTransform(scaleX: 1, y: 2)
//                self.violetLabel.transform = CGAffineTransform(scaleX: 1, y: 2)
