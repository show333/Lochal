...
//        let alert : UIAlertController = UIAlertController(title: "タイトル", message: "\(imageUrls[indexPath.row])", preferredStyle: .alert)
...
//
//        let imageView = UIImageView(frame: CGRect(x:width/4.3,y: -height/3,width: width/2, height:width/2))
////        let imageView = UIImageView(frame: CGRect(x:0,y: -height/3,width: width/2, height:width/2))
...
//        let alert: UIAlertController = UIAlertController(title: "このスタンプを送信しますか？", message: "このスタンプでよろしければOKを押してください", preferredStyle:  UIAlertController.Style.actionSheet)
//
//        // ② Actionの設定
...
//
//            self.addMessageToFirestore(urlString: self.imageUrls[indexPath.row])
//            print(self.imageUrls[indexPath.row])
//            print("OK")
//        })
//        // キャンセルボタン
